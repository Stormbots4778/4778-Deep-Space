...
public class Ascend extends CommandGroup {
    ...
    public Ascend() {
    	...
    	//addSequential(new LiftDrive(0.2,true,0.5)); //Drive forward further onto upper level
    	addParallel(new LiftDrive(0.2,false,0.5));
    	addSequential(new AutoTimer(0.5));
    	
    	addSequential(new LiftRear(false)); //Retract rear lifter
    	addSequential(new AutoTimer(0.5));
    	
    	addSequential(new LiftDrive(0.2,false,0.2)); //Drive all the way onto upper level
	}
}
